    ...
    public void SetHotbarSlotDisplay()
    {
        if (hotbarItemName != Items.None)
        {
            ...
            for (int i = 0; i < InventoryManager.Instance.inventories[0].itemsInInventory.Count; i++)
            {
                ...
                {
                    float tempFill = InventoryManager.Instance.inventories[0].itemsInInventory[i].durability_Current / MainManager.Instance.GetItem(InventoryManager.Instance.inventories[0].itemsInInventory[i].itemName).durability_Max;
